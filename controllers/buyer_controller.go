...
// SelectBuyerInfo 查询买家列表信息
func SelectBuyerInfo(ctx context.Context, r *BuyerReconciler) ([]myappv1.Buyer, error) {
	buyerList := &myappv1.BuyerList{}
	if err := r.List(ctx, buyerList); err != nil {
		log.Log.Error(err, "获取买家列表对象出错")
		return nil, err
	} else {
		log.Log.Info(fmt.Sprintln("获得买家列表对象：", buyerList))
		return buyerList.Items, nil
	}
}
