...
// SelectGoodsInfo 根据商品ID，查询商品列表信息
func SelectGoodsInfo(ctx context.Context, r *SpikeGoodsReconciler) ([]myappv1.SpikeGoods, error) {
	spikeGoodsList := &myappv1.SpikeGoodsList{}
	if err := r.List(ctx, spikeGoodsList); err != nil {
		log.Log.Error(err, "获取商品列表对象出错")
		return nil, err
	} else {
		log.Log.Info(fmt.Sprintln("获得商品列表对象：", spikeGoodsList))
		return spikeGoodsList.Items, nil
	}
}
